...
File_mapleOutput = '/nfs/research/goldman/zihao/Datas/p2_compViridian_P3/Folder_mapleOutput/COL_output_modified.txt'
# For colman
df_col = pd.read_csv(File_mapleOutput, delimiter='\t')
df1 = df_col
...
def find_row(df, position):
    ...
    if row.empty:
        ...
        return row[['REF', 'ALT', 'AF', 'nucleotide_origin']]
...
new_df = pd.DataFrame(new_rows, columns=['REF', 'ALT', 'AF', 'nucleotide_origin'])
