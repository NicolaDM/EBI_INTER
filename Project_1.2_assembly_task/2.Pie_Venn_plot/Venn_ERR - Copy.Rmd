...
# inverse now
data_ERR_same <- read.table("../A_Datas/For_COMP/COMP_ALL_diff.txt", header = TRUE, sep = "\t")
data_ERR_diff <- read.table("../A_Datas/For_COMP/COMP_ALL_same.txt", header = TRUE, sep = "\t")
...
"#A51919",
"#800000"), 
                       values = c(0, 0.01, 0.04, 0.08, 0.1, 0.3, 0.5, 0.7, 0.9, 1), limits = c(0, 35000), guide = "none")+
    scale_color_manual(values = manual_colors)+
    theme(panel.border = element_rect(colour = "white", fill=NA, size=1.5),   # 给图加一个边框
        panel.background = element_rect(fill = "#E04C4C"))+# ,
